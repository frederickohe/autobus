...
- `GET /api/v1/social/connect/{platform}` → Postiz direct OAuth for mapped platforms (`facebook`, `instagram`, `whatsapp`, `tiktok`, `youtube`); requires provider credentials on Postiz (`GET /api/public/v1/social/{slug}`). Instagram tries `instagram` then `instagram-standalone`.
